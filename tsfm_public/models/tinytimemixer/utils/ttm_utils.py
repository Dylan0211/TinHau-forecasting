...
def plot_preds(trainer, dset, plot_dir, num_plots=10, plot_prefix="valid", channel=-1, truncate_history=True):
    ...
    for i, ri in enumerate(random_indices):
        ...
        if truncate_history:
            x = batch["past_values"][-7 * pred_len :, channel].squeeze().cpu().numpy()
        else:
        ...
        # Plot horizon border
        axs[i].axvline(x=7 * pred_len, color="r", linestyle="-")
...
def get_data(
    ...
    if dataset_name not in config_map.keys():
        raise ValueError(
            f"Currently `get_data()` function supports the following raw_dataset: {config_map.keys()}\n \
                         For other raw_dataset, please provide the proper configs to the TimeSeriesPreprocessor (TSP) module."
        )
    """""""""
    ...
    tsp = TimeSeriesPreprocessor(
        **column_specifiers,
        ...
        scaling=True,
        encode_categorical=False,  # here we don't need to encode because our feature is one-hot
        scaler_type="minmax",
    )
