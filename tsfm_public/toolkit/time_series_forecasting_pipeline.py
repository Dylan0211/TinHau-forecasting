...
class TimeSeriesPipeline(Pipeline):
    def run_single(self, inputs, preprocess_params, forward_params, postprocess_params):
        ...
        """
        # our preprocess returns a raw_dataset
...
class TimeSeriesForecastingPipeline(TimeSeriesPipeline):
    ...
    def preprocess(self, time_series, **kwargs) -> Dict[str, Union[GenericTensor, List[Any]]]:
        """Preprocess step
        Load the data, if not already loaded, and then generate a pytorch raw_dataset.
        """
        ...
        # use forecasing raw_dataset to do the preprocessing
        ...
        # for k, v in first.items():
        #     if isinstance(v, torch.Tensor):
        #         full_output[k] = torch.stack(tuple(r[k] for r in raw_dataset))
        #     else:
        #         full_output[k] = [r[k] for r in raw_dataset]
