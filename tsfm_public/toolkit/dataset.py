...
#
"""Tools for building torch raw_dataset"""
...
class BaseDFDataset(torch.utils.data.Dataset):
    ...
    All the raw_dataset that represents data frames should subclass it.
...
class BaseConcatDFDataset(torch.utils.data.ConcatDataset):
    ...
    Args:
        ...
        seq_len (int, required): the sequence length. Defaults to 1
        num_workers (int, optional): the number if workers used for creating a list of raw_dataset from group_ids. Defaults to 1.
        pred_len (int, required): forecasting horizon. Defaults to 0.
        cls (class, required): raw_dataset class
    """
...
class RegressionDFDataset(BaseConcatDFDataset):
    ...
    Args:
        ...
        id_columns (list, optional): List of columns that specify ids in the raw_dataset. list of group_ids to split the data_df to different groups. If group_ids is defined, it will triggle the groupby method in DataFrame. If empty, entire data frame is treated as one group.
