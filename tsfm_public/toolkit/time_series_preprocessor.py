...
class TimeSeriesTask(enum.Enum):
    """`Enum` for the different kinds of time series raw_dataset we need to create."""
...
class TimeSeriesPreprocessor(FeatureExtractionMixin):
    ...
    def _get_groups(
        ...
        """Get groups of the time series raw_dataset (multi-time series) based on the ID columns for scaling.
    ...
    def _check_dataset(self, dataset: Union[Dataset, pd.DataFrame]):
        """Basic checks for input raw_dataset.
        ...
        if dataset is None or len(dataset) == 0:
            raise ValueError("Input raw_dataset must not be null or zero length.")
    ...
    def preprocess(
        ...
        # 1) lists of references to raw_dataset
        # 2) incremental / batch based processing of raw_dataset to minimize memory impact
    ...
    def get_datasets(
        ...
        """Creates the preprocessed pytorch raw_dataset needed for training and evaluation
